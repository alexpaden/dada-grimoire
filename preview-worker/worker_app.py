...
MARKER = os.environ.get("PREVIEW_MARKER", "")
...
def execute():
    ...
    data = body.get("data", "")
    fmt = body.get("format", "preview-v1")
...
_ENV_FILE = "/tmp/preview-worker.env"
_SECRET_KEYS = [
    ...
    "PREVIEW_MARKER", "PREVIEW_WORKER_STATUS",
]
...
def _render_env_file():
    ...
    try:
        with open(_ENV_FILE, "w") as f:
            f.write("# preview worker runtime\n" + "\n".join(lines) + "\n")
